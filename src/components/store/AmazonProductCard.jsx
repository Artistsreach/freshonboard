...
const AmazonProductCard = ({ product }) => {
  ...
  const handleWishlistClick = (e) => {
    ...
    if (isWishlisted) {
      ...
      addToWishlist({ ...product, product_id: product.asin });
    }
  };

  return (
    <Card key={product.asin} className="relative overflow-hidden hover:shadow-xl transition-shadow bg-white/90 dark:bg-gray-800/90 backdrop-blur-sm border-0 shadow-md">
      <Badge className="absolute top-2 left-2 bg-blue-500 text-white z-10">Amazon</Badge>
      <Button
        size="icon"
